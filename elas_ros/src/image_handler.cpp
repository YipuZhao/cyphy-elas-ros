...
_it(_nh)
{
    ...
    //
    std::string rectImgTopicName;
    std::string rectInfoTopicName;
    ...
    while(!(_nh.getParam("topic_crop", imgTopicName) &&
          _nh.getParam("topic_rect", rectImgTopicName) &&
          _nh.getParam("camera_info", infoTopicName) &&
          _nh.getParam("camera_info_rect", rectInfoTopicName)) &&
	  ros::ok())
    {
	ros::spinOnce();
	wrait.sleep();
  ...
  //  _pub_info = _nh.advertise<sensor_msgs::CameraInfo>(infoTopicName + "_crop", 1);
    ...
    _pub_img = _it.advertise(rectImgTopicName, 1);
    _pub_info = _nh.advertise<sensor_msgs::CameraInfo>(rectInfoTopicName, 1);
...
void SingleImageHandler::topicCallback(const sensor_msgs::ImageConstPtr& received_image)
{
    ...
    // std::cout << "done with resizing" << std::endl;
